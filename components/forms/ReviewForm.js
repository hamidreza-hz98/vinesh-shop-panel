...
export default function ReviewForm({ data, onClose, onSuccess }) {
  ...
  const onSubmitReply = async (formData) => {
    ...
    // Add reply to review
    onSuccess && onSuccess({ reply: formData.reply, status });
    reset({ reply: "" });
  };
  ...
  return (
    <Box sx={{ width: "100%" }}>
      <Grid container spacing={1}>
        <Grid size={{ xs: 12 }} display="flex" alignItems="start" justifyContent="space-between" >
        ...
        >
          ...
          <Link href={`dial: ${data.user.phoneNumber}`}>
...

        {
          ...
          <Grid mt={1} size={{ xs: 12 }} display="flex" alignItems="center" gap={2} >
          {
        ...
        <Grid size={{ xs: 12 }} direction="row" display="flex" alignItems="center"  justifyContent="space-between" >
          <Button
            variant="contained"
